...
// let previousPage = ""; // 이 변수는 아래 pageStack으로 대체됩니다.
let pageStack = []; // 뒤로가기를 위해 페이지 스택을 사용합니다.


// const pages 배열은 그대로 사용합니다.
const pages = ["mainPage", "subjectPage", "clubIntroPage", "surveyPage", "resultPage"];

// scoreTable과 preferenceWeight는 전역 변수로 그대로 둡니다.
const scoreTable = {
...
    pageStack.push(id);
  }
  currentPage = id; // 현재 페이지를 업데이트합니다.
}
...
  const titleMap = {
    math: '수학 동아리', physics: '물리 동아리', chemistry: '화학 동아리',
    earth: '지구과학 동아리', bio: '생물 동아리', cs: '정보 동아리'
  };
  document.getElementById("subject-title").innerText = titleMap[subject] || "";

  const list = document.getElementById("clubList");
  list.innerHTML = "";
  (clubData[subject] || []).forEach(c => {
    const btn = document.createElement("button");
    btn.className = "clubButton";
    btn.innerText = c.name;
    btn.addEventListener('click', () => showClubIntro(subject, c.name));
    list.appendChild(btn);
  });
}

function showClubIntro(subject, name) {
  showPage("clubIntroPage");
  const club = (clubData[subject] || []).find(c => c.name === name);
  const box = document.getElementById("clubIntroBox");
  box.className = "club-card"; // 이전 CSS의 club-card 클래스를 유지
  box.innerHTML = club
    ? `<h2>${club.name}</h2><p>${club.desc}</p>`
    : `<p>정보 없음</p>`;
}

function startSurvey() { showPage("surveyPage"); }
function resetSurvey() { document.getElementById('surveyForm').reset(); }

function showResult() {
  const form = document.forms['surveyForm'];
  // 아래 유효성 검사 추가: 모든 라디오 버튼이 선택되었는지 확인
  if (!form.subject.value || !form.senior.value || !form.dining.value || !form.difficulty.value) {
    alert("모든 설문 문항에 응답해주세요!");
    return;
  }

  const subject = form.subject.value;
  const activities = Array.from(form.activities).filter(a => a.checked).map(a => a.value);
  const senior = form.senior.value;
  const dining = form.dining.value;
  const difficulty = form.difficulty.value;

  const maxScore = 313; // 최대 점수 기준
  const scores = [];
...
    scores.push({ club, matchRate: Math.round(score / maxScore * 100) });
  }

  // 점수 계산 후 결과가 3개 미만일 경우 처리 (여기서는 모든 동아리가 계산되므로 필요 없을 수 있지만, 혹시 몰라 추가)
  if (scores.length < 3) {
    alert("결과를 계산할 수 없습니다. 동아리 데이터가 부족합니다.");
    return;
  }
...
function goBack() {
  pageStack.pop(); // 현재 페이지를 스택에서 제거
  const prev = pageStack.pop() || "mainPage"; // 이전 페이지를 가져오고, 없으면 mainPage로
  showPage(prev); // 이전 페이지를 보여줍니다.
}
...
      const checked = Array.from(document.querySelectorAll('input[name="activities"]')).filter(c => c.checked);
      if (checked.length > 2) {
        cb.checked = false;
